...
export default function Library({ currentUser, onPlayTrack }) {
  ...
  const fetchLibraryData = async () => {
    ...
    try {
      const [favRes, plRes] = await Promise.all([
        axios.get(`http://localhost:5000/api/favorites/all?username=${currentUser.username}&_t=${Date.now()}`),
        axios.get(`http://localhost:5000/api/playlists/all?username=${currentUser.username}&_t=${Date.now()}`)
      ]);
  ...
  const handleDeletePlaylist = async (playlistId) => {
    ...
    try {
      await axios.post("http://localhost:5000/api/playlists/delete", { playlist_id: playlistId });
  ...
  const handleTogglePin = async (playlistId) => {
    try {
      const res = await axios.post("http://localhost:5000/api/playlists/toggle_pin", { playlist_id: playlistId });
      if (res.data?.success) {
  ...
  const handleRenamePlaylist = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/playlists/update_name", {
  ...
  const handleDragSort = async () => {
    ...
    // Persist to backend
    try {
      await axios.post("http://localhost:5000/api/playlists/reorder", {
        playlist_id: activePlaylist._id,
        tracks: tracksCopy
      });
      window.dispatchEvent(new Event('libraryUpdate'));
                    ...
                    <button
                      onClick={async (e) => {
                        ...
                        if (window.confirm("Remove this song from the playlist?")) {
                          try {
                            await axios.post("http://localhost:5000/api/playlists/remove_track", {
                              playlist_id: _id,
